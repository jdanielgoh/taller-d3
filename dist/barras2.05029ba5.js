...
function barras2() {
  ...
  var alto = ancho * .5 - margin.top - margin.bottom; //Creamos el SVG

  var svg = d3.select("#barras-2").append("svg").attr("width", ancho + margin.left + margin.right).attr("height", alto + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); //Ire creando  funciones que me ayudaran a reescalar los datos, 
  //Mas adelante les atribuire más especificaciones
  //Formato temporal

  var parseDate = d3.timeParse("%Y-%m-%d"); //Escalas

  var escalaX = d3.scaleTime().range([0, ancho]);
  var escalaY = d3.scaleLinear().range([alto, 0]);
  var escalaRangeBand = d3.scaleBand().range([0, ancho]).padding(.2); //Cargamos un csv
  ...
  d3.csv("../../BaseEstadosPositivosyDefunciones.csv").then(function (ladata) {
    ...
    var data = ladata.filter(function (d) {
      return d.Estado == "Ciudad de México";
    });
    console.log(data); //Ajustamos dominios de escalas

    escalaX.domain(d3.extent(data.map(function (d) {
      return d.fecha;
    })));
    escalaY.domain(d3.extent(data.map(function (d) {
      return d.Positivos;
    })));
    escalaRangeBand.domain(data.sort(function (a, b) {
      return a.fecha - b.fecha;
    }).map(function (d) {
      return d.fecha;
    })); //Agregamos los ejes 

    var ejeX = svg.append("g").attr("transform", "translate(0," + alto + ")").call(d3.axisBottom(escalaX).tickFormat(d3.timeFormat("%d-%m-%Y"))).attr("class", "ejex");
    var ejeY = svg.append("g").call(d3.axisLeft(escalaY).ticks(5)).attr("class", "ejey"); //Creamos las barras de positivos

    var barras = svg.selectAll(".barras").data(data).enter().append("rect").attr("x", function (d) {
      return escalaRangeBand(d.fecha);
    }).attr("y", function (d) {
      return escalaY(d.Positivos);
    }).attr("height", function (d) {
      return escalaY(0) - escalaY(d.Positivos);
    }).attr("width", escalaRangeBand.bandwidth()).style("fill", "#017a8c"); //Creamos una linea con d3.line

    var linea = svg.selectAll(".linea").data([data]).enter().append("path").attr("d", function (d) {
      return d3.line().x(function (dd) {
        return escalaX(dd.fecha);
      }).y(function (dd) {
        return escalaY(+dd.Positivos);
      })(d);
    }).attr("class", "linea").style("stroke-width", "1.5px").style("fill", "none").style("stroke", "#018c52"); /// Explorando transiciones y eventos:

    svg.on("click", function (event, dd) {//barras.transition().duration(1000).style("fill","#c48600")
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/barras/barras2.js"], null)
//# sourceMappingURL=/barras2.05029ba5.js.map