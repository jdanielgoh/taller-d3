...
function visualizacionPaths() {
  ...
  var alto = ancho * .9 - margin.top - margin.bottom; //Creamos el SVG

  var svg = d3.select("#figuras-2").append("svg").attr("width", ancho + margin.left + margin.right).attr("height", alto + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); //Formato de tiempo
  ...
  d3.csv("../../hopkinsDefunciones_paravis2.csv").then(function (data) {
    ...
    var keys = Object.keys(dict_cats); //creando escalas

    var escalaX = d3.scaleTime().domain(d3.extent(data, function (d) {
      return d.fech;
    })).range([0, ancho]);
    var escalaY = d3.scaleLinear().domain([0, d3.max(data.map(function (d) {
      return d.totoD;
    }))]).range([alto, 0]); //AGregando ejes  

    svg.append("g").call(d3.axisLeft(escalaY)).attr("class", "ejey eje");
    svg.append("g").attr("transform", "translate(0," + alto + ")").call(d3.axisBottom(escalaX).tickFormat(d3.timeFormat("%d-%m-%Y"))).attr("class", "ejex eje").selectAll("text").style("text-anchor", "end").attr("dx", "-.8em").attr("dy", ".15em").attr("transform", "rotate(-65)"); //Escala ordinal

    var color = d3.scaleOrdinal().domain(keys).range(Object.values(dict_cats)); //Apilando la data

    var stackedData = d3.stack().offset(d3.stackOffsetNone).keys(keys)(data); //Aqui me invento un pedazo de codigo para reordenar los datos de las franjas, manteniendo siempre
    // los anchos (deltas)
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/figuras/figuras-2.js"], null)
//# sourceMappingURL=/figuras-2.74214080.js.map