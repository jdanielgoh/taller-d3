...
function scatterplot() {
  ...
  var svg1 = d3.select("#scatterplot").append("svg").attr("width", ancho + margin.left + margin.right).attr("height", alto + margin.top + margin.bottom);
  var svg_defs = svg1.append("defs");
  var linearGradient = svg_defs.append("linearGradient").attr("id", "degradado").attr("x1", "0%").attr("y1", "0%").attr("x2", "100%").attr("y2", "0%");
  linearGradient.append("stop").attr("offset", "0%").attr("style", "stop-color:#66D7D1;stop-opacity:1");
  linearGradient.append("stop").attr("offset", "100%").attr("style", "stop-color:#FC7753;stop-opacity:1");
  var svg = svg1.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); //Formato de tiempo

  var parseDate = d3.timeParse("%Y-%m-%d"); //Escalas

  var escalaX = d3.scaleTime().range([0, ancho]);
  var escalaY = d3.scaleLinear().range([alto, 0]); //Abrimos datos
  ...
  d3.csv("../../BaseEstadosPositivosyDefunciones.csv").then(function (ladata) {
    ...
    var circulos = svg.selectAll("circulos").data(data).enter().append("circle").attr("cx", function (d) {
      return escalaX(d.fecha);
    }).attr("cy", function (d) {
      return escalaY(d.Positivos);
    }).attr("r", function (d) {
      return Math.sqrt(d.Defunciones);
    ...
    var ejeY = svg.append("g").call(d3.axisLeft(escalaY).ticks(5)).attr("class", "ejey");
    ...
    function reescalado() {
      ...
      svg1.attr("width", ancho + margin.left + margin.right).attr("height", alto + margin.top + margin.bottom);
      escalaX.range([0, ancho]);
      escalaY.range([alto, 0]);
      ejeX.attr("transform", "translate(0," + alto + ")").call(d3.axisBottom(escalaX).tickFormat(d3.timeFormat("%d-%m-%Y")));
      ejeY.call(d3.axisLeft(escalaY).ticks(5));
      circulos.attr("cx", function (d) {
        return escalaX(d.fecha);
      }).attr("cy", function (d) {
        return escalaY(d.Positivos);
      });
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/barras/scatterplot.js"], null)
//# sourceMappingURL=/scatterplot.0aefeb28.js.map