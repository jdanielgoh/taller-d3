...
function fuerzas1() {
  ...
  var svg = d3.select("#fuerzas-1").append("svg").style("background", "#353336").attr("width", ancho + margin.left + margin.right).attr("height", alto + margin.top + margin.bottom).append("g").attr("translate", "translate(".concat(margin.left, ",").concat(margin.top, ")"));
  var escalas_radios = .002 * ancho,
      nodePadding = 2.;
  d3.csv("../../hopkinsDefunciones_paravis.csv").then(function (ladata) {
    ...
    for (var i = 0; i < paises.length; i++) {
      if (+ladata.slice(-1)[0][paises[i]] != 0) {
        data.push({
          pais: paises[i],
          valor: +ladata.slice(-1)[0][paises[i]]
        });
      }
    ...
    */
    ...
    var burbujas = svg.selectAll(".circulos").data(data).enter().append("circle").attr("class", "circulos").attr("r", function (d) {
      return d.radio;
    }).style("fill", "#00ab89").style("stroke", "#be27e8").style("stroke-width", "2px").style("stroke-dasharray", "3 2");
    var simulation = d3.forceSimulation(data).force('charge', d3.forceManyBody().strength(-10)).force('x', d3.forceX().x(function (d) {
      return ancho * .5;
...
},{}]},{},["node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/fuerzas/fuerzas1.js"], null)
//# sourceMappingURL=/fuerzas1.8c568b36.js.map